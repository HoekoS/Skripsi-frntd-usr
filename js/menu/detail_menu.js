  ...
  res => {
    res.json().then(
      data => {
        ...
        document.getElementById('imageMenu').src='http://file.cafesako.store/'+data.data[0].pic;
      }
    )
  }
)
...
function postPesanan(){
  ...
  .then(data=>{
    if(data.status=="True"){
      ...
      // alert("sukses")
      window.location.replace('http://cafesako.store/menu_all.html?not='+nomer_nota+'&nom='+nomer_meja);
    }else{
